...
class MultiWriterRoom extends ReadyResource {
  ...
  async _open () {
    ...
    this.pairMember = this.pairing.addMember({
      ...
      onadd: async (request) => {
        ...
        const inv = await this.view.findOne(`@${this.dbNamespace}/invites`, {})
  ...
  _addRouter () {
    ...
    this.router.add(`@${this.dbNamespace}/add-invite`, async (data, context) => {
      await context.view.insert(`@${this.dbNamespace}/invites`, data)
    })
    this.router.add(`@${this.dbNamespace}/del-invite`, async (data, context) => {
      await context.view.delete(`@${this.dbNamespace}/invites`, { id: data.id })
    })
    this.router.add(`@${this.dbNamespace}/add-user`, async (data, context) => {
      await context.view.insert(`@${this.dbNamespace}/users`, data)
    })
    this.router.add(`@${this.dbNamespace}/del-user`, async (data, context) => {
      await context.view.delete(`@${this.dbNamespace}/users`, { id: data.id })
    })
  }
  ...
  async createInvite () {
    const existing = await this.view.findOne(`@${this.dbNamespace}/invites`, {})
    if (existing) {
  ...
  async getUsers () {
    return await this.view.find(`@${this.dbNamespace}/users`, {}).toArray()
  }
