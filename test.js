...
test('event', async t => {
  ...
  const eventPromise = rrp()
  roomWriter2.on('new-event', async () => {
    const events = await getEvents(roomWriter2)
    eventPromise.resolve(events)
  })

  await roomWriter1.addEvent('event1', { hello: 'world', at: new Date().toISOString() })

  const events = await eventPromise.promise
  t.is(events.length, 1, 'should have one event')
  t.is(events[0].id, 'event1', 'correct event id')
  t.is(events[0].data.hello, 'world', 'correct event info')
...
async function getEvents (room) {
  const events = await room.getEvents()
  if (events.length) return events

  await new Promise(resolve => setTimeout(resolve, 100))
  return getEvents(room)
}
